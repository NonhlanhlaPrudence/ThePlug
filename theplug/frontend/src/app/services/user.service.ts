...
export class UserService {
  ...
  get_user_by_id (user_id: string): Observable<any> {
    return this.httpClient
      .get(`${this.SERVER_URL}/users/get_user/${user_id}`, {
  ...
  sign_up (user: User): Observable<any> {
    return this.httpClient
      .post(`${this.SERVER_URL}/users/register_users`, user);
  }

  // login
  sign_in (email: string, password: string) {
    return this.httpClient.post<any>(`${this.SERVER_URL}/users/login_users`, { email, password });
  }
